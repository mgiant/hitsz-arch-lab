...
通过计数器实现LRU调度算法，在选中一行时该行计数器置零，其余行递增。LRU比较器返回当前计数最大的行

![image-20200609004639251](img\image-20200609004639251.png)

命中判断电路，比较各自行的`TagData`与输入信号的`Tag`，若相等则`TL`(Temp L0)为1

![image-20200609004651707](img\image-20200609004651707.png)

如果没有命中的行(`Miss` = 1)，则通过LRU比较电路生成行选信号

![image-20200609004806408](img\image-20200609004806408.png)



行选择电路，通过各行的比较结果判断是否有命中，只要有一行比较结果相等，`Miss`就为0因为该Cache只实现了读，valid bit在Cache初始化后就一直为1，此处省略了`Valid`。![image-20200609004943392](.\img\image-20200609004943392.png)

完成全相联电路后，在DMEMECache中将直接相联Cache部件替换为全相联。最终运行结果如下

![image-20200609111243611](img/image-20200609111243611.png)
